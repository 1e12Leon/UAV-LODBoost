...
class YOLO(object):
    _defaults = {
        ...
        #--------------------------------------------------------------------------#
        "model_path"        : 'model_data/v1_sw(1).pth',
    ...
    def generate(self, onnx=False):
        ...
        #---------------------------------------------------#
        self.net    = YoloBody(self.anchors_mask, self.num_classes, self.phi, pretrained=False, phi_attention=1)
    ...
    # ---------------------------------------------------#
    #   检测图片,返回dets，用于跟踪算法的输入
    # ---------------------------------------------------#
    def detect_image_dets(self, image):
        # ---------------------------------------------------#
        #   计算输入图片的高和宽
        # ---------------------------------------------------#
        image_shape = np.array(np.shape(image)[0:2])
        ...
        for i, c in list(enumerate(top_label)):
            ...
            dets.append([left, top, right, bottom, score])
            """label = '{} {:.2f}'.format(predicted_class, score)
            draw = ImageDraw.Draw(image)
            label_size = draw.textsize(label, font)
            label = label.encode('utf-8')"""
