...
def add_xml(xml_path,image_name,xml_save_path,add_object_xml,SMB_image_name,image_save_path,img_h, img_w):
    ...
    fp.close()
def collect_small_object(xml_path,SOA_THRESH):
    """
    目的：提取图片中所有小目标
    ...
    for xml_name in xmls:
        ...
        for obj in tree1.findall("object"):
            ...
            if issmallobject(SOA_THRESH,bbox_h,bbox_w):
...
def Samll_object_Augmentation(image_path,xml_path,image_save_path,xml_save_path,SOA_THRESH, SOA_PROB, SOA_COPY_TIMES, SOA_EPOCHS):
    image_names = os.listdir(image_path)
    print('----------------------------')
    print('小目标数据增强')

    c_w, c_h = 1980, 1080
    small_object_list = collect_small_object(xml_path, SOA_THRESH)
    for image_name in tqdm(image_names):
        ...
        small_bbox=[int(small_object_list[i][0]*rate_w_2),int(small_object_list[i][1]*rate_h_2),int(small_object_list[i][2]*rate_w_2),int(small_object_list[i][3]*rate_h_2),small_object_list[i][4]]
