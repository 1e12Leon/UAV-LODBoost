...
from utils.utils_fit import fit_one_epoch, fit_one_epoch_distill
...
if __name__ == "__main__":
    ...
    t_model_path = 'model_data/yolov7_weights.pth'
    # ------------------------------------------------------#
    ...
    if distill:
        ...
        #   加载教师模型
        # ------------------------------------------------------#
        t_model = YoloBody(anchors_mask, num_classes, t_phi, pretrained=pretrained)
        if t_model_path != '':
            # ------------------------------------------------------#
            #   权值文件请看README，百度网盘下载
            # ------------------------------------------------------#
            if local_rank == 0:
                print('Load teacher weights {}.'.format(t_model_path))
    ...
    if Cuda:
        if distributed:
            ...
            if distill:
                t_model_train = torch.nn.DataParallel(t_model)
                cudnn.benchmark = True
                t_model_train = t_model_train.cuda()
    ...
    if True:
        ...
        for epoch in range(Init_Epoch, UnFreeze_Epoch):
            ...
            if distill:
                fit_one_epoch_distill(model_train, t_model_train, model, ema, yolo_loss, loss_history, eval_callback, optimizer, epoch,
                              epoch_step,
