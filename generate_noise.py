...
if __name__ == "__main__":
    ...
    for img_name in os.listdir(Origin_JPEGImages_path):
        ...
        img_dst_path = os.path.join(Out_JPEGImages_path, name+'_noise.jpg')
        img = cv2.imread(img_src_path)
        xml_src_path = os.path.join(Origin_Annotations_path, name+'.xml')
        xml_dst_path = os.path.join(Out_Annotations_path, name+'_noise.xml')
        ...
        # 修改xml文件中的filename
        dom = parse(xml_dst_path)
        root = dom.getroot()
        for ann in root.iter('annotation'):
            ann.find('filename').text = img_dst_path

        dom.write(xml_dst_path)
